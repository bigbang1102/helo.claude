...
### 2026-07-23 — เพิ่ม BDNAME `POSOB5` เข้า `set_display` (สำเนา POSOB ทั้งหมด)

- คำสั่ง user: POSOB5 ใช้ layout เหมือน POSOB ทุกอย่าง — ให้เพิ่ม BDNAME เข้าไปใน `set_display` โดยไม่ต้องแก้ master/item อื่นเลย (แก้เฉพาะ `set_display`)
- **หมายเหตุ concurrency**: ช่วงนี้ Codex ทำงานอยู่กับไฟล์คนละไฟล์ (`T-POS-changePOSOutbounddelivery.xls`, ~113KB) ส่วนงานนี้แก้ที่ `new_Transaction\T-POS-POSOutbounddelivery.xls` (~2.5MB) ตัวเต็ม — คนละไฟล์กัน ไม่ชนกัน แต่ **ควรถาม user ก่อนเสมอว่า Codex กำลังแตะไฟล์เดียวกันอยู่ตอนนี้ไหม** ก่อนเปิดไฟล์ excel-gen ที่ทำงานคู่กันแบบนี้
- backup ไว้ก่อนแก้: scratchpad `T-POS-POSOutbounddelivery.backup-before-posob5.xls`
- ยืนยันรูปแบบ multi-BDNAME จาก `T-FI-AR-newtab_prae.xls` ก่อนลงมือ (AR1-AR7 ใช้ `#delete-bdname-list#` เป็น comma-separated list ครบทุกชื่อ, แต่ละ BDNAME มีแถวข้อมูลของตัวเองแยกกัน) — เป็น pattern เดียวกับที่ต้องทำกับ POSOB5
- วิธีทำ: insert แถวใหม่ต่อท้ายกลุ่ม POSOB เดิมในแต่ละบล็อก (ไม่แก้ทับ) แล้วคัดลอกทุกคอลัมน์จาก POSOB มาเป็น POSOB5 (เปลี่ยนแค่คอลัมน์ BDNAME) + อัปเดต `#delete-bdname-list#` ทั้ง 4 บล็อกเป็น `POSOB,POSOB5`
- ผลลัพธ์: `set_display` จาก 44 แถว → 66 แถว (เพิ่ม 22 = 1+16+1+4 ตามจำนวนแถว POSOB เดิมในแต่ละบล็อก) — block1 (wfdisplayitemdisplaytab) +1, block2 (wfdisplayitemsubtab) +16, block3 (wfdisplay_displayattributeconf) +1, block4 (wfdisplay_displayoptionconf) +4
- verify: spot-check ทุกคอลัมน์ระหว่างแถว POSOB ต้นฉบับกับ POSOB5 ที่ insert ใหม่ ตรงกันหมด (ต่างแค่ col BDNAME) + เช็ค `advance_usecase` ยัง `#REF!=0` ทั้ง 1,173 แถวหลังแก้ (set_display เป็น literal ล้วน ไม่ใช่สูตร ไม่กระทบ sheet อื่น)
- **สถานะ: POSOB5 เพิ่มใน set_display เสร็จแล้ว รอ gen** (ไฟล์เดียวกับ POSOB ที่ status GEN-READY อยู่ก่อนหน้า)
...
## เคส: T-POS-changePOSOutbounddelivery / CPOSOB (Change Doc POSOB, 2026-07-23)

- source: `Transaction\T-POS-changePOSOutbounddelivery.xls` · output: `new_Transaction\T-POS-changePOSOutbounddelivery.xls`
- layout: `POSChangeOB.xlsx` sheet `POS ChangeOB`; source ไม่มี Item block จึงคงเป็น master-only และสร้าง `set_display` สำหรับล้าง item config เก่าของ `CPOSOB`
- Header เดิม 26 field → 27 field โดยเพิ่ม `businessentity`; ลำดับจริง: ประเภทรายการ 3 → การมอบหมาย 2 → คู่ค้าใบกำกับภาษี 9 → ผู้ใช้ 6 → รายละเอียดPOS 3 → ข้อมูลอ้างอิง 4
- default-field 7 ตัว: `document_category` รับ `TXDATE,REFDOCNO,REFDOCDATE,TXNO,STATUS` ตามลำดับในแบบ · `outlet` รับ `REMARK` · `parent_refdocno` รับ `TXTYPE`
- แบบระบุ `REFDOCNO/REFDOCDATE` ซ้ำในข้อมูลหลักและข้อมูลภายใน แต่ห้ามสร้าง field/default ซ้ำเพราะ redTools จะเกิด `Field Duplicate`; ใช้ตำแหน่งข้อมูลหลักตาม grid แถวแรก/แถวสองเพียงตำแหน่งเดียว
- ทุก Header ใช้ natural wrap 4 คอลัมน์ (`#new-line#` ว่างทั้งหมด)
- extend-master: `รายละเอียดPOS`, `ข้อมูลภายใน`
- correction readonly 2026-07-23: กฎของชุดแบบนี้คือแท็บ `ข้อมูลผู้ใช้`, `รายละเอียดPOS`, `ข้อมูลภายใน` ต้อง readonly **ทุก field** ไม่ใช่เฉพาะข้อมูลอ้างอิง
- CPOSOB readonly master ที่ถูกต้อง 13 field ตามชื่อใน `wfdisplay_displaymasterconf`: `user_owner,role_owner,createbyCode,createtime,user_destination,role_destination,posidCode,businessplace_for_txno,posob_txno,flow_refdoctype,flow_refdocno,parent_refdoctype,parent_refdocno`
- correction 2026-07-23: `FIELDLIST` ของ `wfdisplay_displayattributeconf` ต้องใช้ชื่อ field ที่ UI สร้างจริง; field ชนิด Reference ใช้ suffix `Code` (`createbyCode`, `posidCode`) ไม่ใช่ชื่อฐานจาก `standard_usecase` มิฉะนั้น config readonly ของทั้งแถวอาจไม่ถูกใช้
- verifier ต้องเทียบ `wfdisplay_displayattributeconf.FIELDLIST` กับ union ของ field ทั้ง 3 แท็บ ห้ามตรวจเพียงว่ามีแถว readonly หรือมี field ข้อมูลภายในครบ
- offset เฉพาะไฟล์นี้: advance master row = standard row +21; เพิ่ม slot ที่ 27 โดย clone pattern แถวท้ายและแก้ reference ไปยัง standard/constrain/formula/other แถวใหม่
- verify: master 27 · formulas 631 · `#REF!` 0 · external links 0 · NPOI และ xlrd เปิดอ่านครบ 8 sheets
# Correction: CPOSOB readonly runtime alias (2026-07-23)

- อาการ: `ข้อมูลผู้ใช้`, `รายละเอียดPOS`, `ข้อมูลภายใน` ของ Change Doc POSOB ยังแก้ไขได้ แม้มี `CPOSOB | master | readonly` และ FIELDLIST สะกดตรงแล้ว
- หลักฐาน DOM: field จริงคือ `user_owner`, `role_owner`, `createbyCode`, `createtime`, `user_destination`, `role_destination`, `posidCode`, `businessplace_for_txno`, `posob_txno`, `flow_refdoctype`, `flow_refdocno`, `parent_refdoctype`, `parent_refdocno`
- หลักฐาน runtime:
  - POS DayEnd: `requestName=PDE`, `txType=PDE` → readonly row `PDE` ทำงาน
  - Change Doc POSOB: `requestName=changeposoutbounddelivery`, `txType=''` → readonly row `CPOSOB` ไม่ถูกโหลด
- วิธีแก้ที่ยืนยันด้วย UI จริง: คง row `CPOSOB` และเพิ่ม alias row `changeposoutbounddelivery | master | readonly` ด้วย FIELDLIST เดียวกัน
- ผล: ทั้ง 3 แท็บเป็น read-only สำเร็จหลังเปิดหน้าใหม่/`Ctrl+F5`; ไม่ต้องสร้าง `wfinputvalidation` หรือ `wfinputvalidationitem`
- ความผิดพลาดระหว่างวิเคราะห์: เคยเสนอเพิ่มแถวแม่ `wfinputvalidation ID=21498`; restart แล้วไม่ช่วย จึงลบแถวทดลองคืน ห้ามใช้แนวทางนี้ซ้ำสำหรับ static readonly ระบบใหม่
- ไฟล์ `new_Transaction\T-POS-changePOSOutbounddelivery.xls` ถูกอัปเดตให้ `set_display` มี readonly ทั้ง `CPOSOB` และ runtime alias เพื่อไม่ให้ Generate ครั้งหน้าทับหาย
